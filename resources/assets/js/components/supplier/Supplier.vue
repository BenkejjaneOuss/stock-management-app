        ...
        <div class="row page-titles">
            <div class="col-md-12 align-self-center">
                <div class="d-flex justify-content-end align-items-center">
                    <button type="button" class="btn btn-info d-lg-block m-l-15" data-toggle="modal" data-target="#add-supplier" @click="cancel"><i class="fa fa-plus-circle"></i> New supplier</button>
                    <input type="hidden" class="btn btn-info d-lg-block m-l-15" data-toggle="modal" data-target="#add-supplier" ref="showModal" >
                        ...
                        <table id="example23" class="display nowrap table table-hover table-striped" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                <tr v-for="(supplier, index) in suppliers">
                                    ...
                                    <td>
                                    <button type="submit" @click="onEdit(supplier)" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn"><i class="ti-pencil-alt" aria-hidden="true"></i></button>
                                    <button type="submit" @click="onDelete(supplier)" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn"><i class="ti-close" aria-hidden="true"></i></button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
    ...
    methods:{
        ...
        onCreate() {
            this.$validator.validateAll().then(result => {
                if (!this.errors.any()) {
                    this.loading = true
                    axios.post(route('supplier.add'), this.form).then(res => {
                        ...
                        if(res.data.result) {
                            this.suppliers.unshift(res.data.supplier);
                            type="success"
                            title = 'Added!'
                            text = "Your supplier has been added."
                            this.form = {
                                name:'',
                                email:'',
                                phone:'',
        ...
        onUpdate() {
            this.$validator.validateAll().then(result => {
                if (!this.errors.any()) {
                    this.loading = true
                    axios.put(route('supplier.update'), this.form).then(res => {
                        ...
                        if(res.data.result) {
                            type="success"
                            title = 'Updated!'
                            text = 'Your supplier has been updated.'
                            this.form = {
                                name:'',
                                email:'',
                                phone:'',
        ...
        onDelete(supplier) {
            this.$swal.fire({
                ...
                if (result.value) {
                    axios.delete(route('supplier.delete', supplier.id)).then(res => {
                        ...
                        if(res.data.result) {
                            var position = this.suppliers.indexOf(supplier);
                            this.suppliers.splice(position, 1);
